...
def Usage():
    print(
        "python Make_transcript_gff.py <<input file path + name>> <<output file Path>>"
    )
...
def Parsing(file):
    ...
    infile = open(file, "r")
    outfile = open(
        oup + "out_transcript_level_" + file[file.rfind("/") + 1 :], "w"
    )
    header = "ID\tGeneID\tCGNC\tfeature_type\tgene\tgenes_synonym\tproduct\ttranscript_id\tprotein_id"
    outfile.write(header)
    for line in infile:
        if not line.startswith("#"):
            Ln = line.strip().split("\t")
            Ln2 = Ln[8].strip().split(";")[0]
            if Ln2.startswith("ID=rna"):
...
def RNA(file):
    ...
    if [i for i in range(len(Ln)) if "GeneID" in Ln[i]]:
        GeneID = Ln[[i for i in range(len(Ln)) if "GeneID" in Ln[i]][0]][
            ...
            + 7 : Ln[[i for i in range(len(Ln)) if "GeneID" in Ln[i]][0]].find(
                ","
            )
        ]
    else:
    ...
    if [i for i in range(len(Ln)) if "CGNC" in Ln[i]]:
        if (
            Ln[[i for i in range(len(Ln)) if "CGNC" in Ln[i]][0]].rfind(",")
            == -1
        ):
            CGNC = Ln[[i for i in range(len(Ln)) if "CGNC" in Ln[i]][0]][
                Ln[[i for i in range(len(Ln)) if "CGNC" in Ln[i]][0]].find(
                    "CGNC:"
                )
                + 5 : Ln[
                    [i for i in range(len(Ln)) if "CGNC" in Ln[i]][0]
                ].rfind(",")
            ]
        else:
            CGNC = Ln[[i for i in range(len(Ln)) if "CGNC" in Ln[i]][0]][
    ...
    if [i for i in range(len(Ln)) if "gene_synonym" in Ln[i]]:
        ...
        gene_synonym = "."
    feature_type = gbkey
    data_lst += ID, GeneID, CGNC, feature_type, gene, gene_synonym
    ...
    if [i for i in range(len(Ln)) if "transcript_id" in Ln[i]]:
        trans_id = Ln[
            ...
            Ln[[i for i in range(len(Ln)) if "transcript_id" in Ln[i]][0]].find(
                "="
            )
            + 1 :
        ]
